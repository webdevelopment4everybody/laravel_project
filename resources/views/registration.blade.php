...
@section('content')
    <div class="mx-auto text-black container relative">
        <h1 class="flex items-center text-5xl font-extrabold mt-[50px] mb-[50px] justify-center dark:text-[#2c384d]">{{__('content.register.reg_title')}}</h1>
        <form class="max-w-md mx-auto" method="post" action="{{route('register.user')}}">
            @csrf
        ...
        @if(session()->has('message'))
            ...
            @if(session()->has('status'))
                <a href="{{route('login')}}">{{__('content.register.login_link')}}</a>
            @endif
            </div>
        @endif
    </div>
